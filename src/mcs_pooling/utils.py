...
def load_dataset(dataset_name, max_nodes):
    if dataset_name == 'PROTEINS':
        dataset = TUDataset(
            root=data_dir + f'/{dataset_name}',
            name=f'{dataset_name}',
            transform=T.Compose([T.Constant(1), T.ToDense(max_nodes)]),
            pre_filter=lambda data: data.num_nodes <= max_nodes,
            force_reload=True
        ).shuffle()
    else: raise ValueError(f'dataset {dataset_name} is not configured for mcs_pooling experiment')
